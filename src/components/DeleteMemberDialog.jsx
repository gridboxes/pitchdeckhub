...
export default function DeleteMemberDialog({ memberName, deckCount = 0, onConfirm, onCancel, loading, dark }) {
  const c = theme(dark)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm p-6 text-center" style={{
        ...
        <h3 className="text-sm font-semibold tracking-tight mb-1" style={{ color: c.text }}>Delete profile?</h3>
        <p className="text-xs mb-6" style={{ color: c.muted }}>
          <span className="font-semibold" style={{ color: c.text }}>{memberName}</span> will be permanently deleted
          {deckCount > 0 && (
            <> and removed from {deckCount} {deckCount === 1 ? 'deck' : 'decks'}</>
          )}.
        </p>
